function DONE =  run_Pairing_analysis(experimenter_ID, recording_date, do_plotting, varargin)
clc
DONE = 0;
p = varargin;
overwrite = p{7};
...
switch experimenter_ID
    ...
    case 'Liselot'
        data_path = os.path.join(data_path_root, '5. LTP electric', recording_date{1}, [recording_date{1},'.data']);
    otherwise
        keyboard 
end
...
% outwave_identifier = 'es';
%% Run analysis
AFTER =[];%NaN(length(GC.current_steps.(experimenter_ID)),1);
amp_AP = [];
width_AP = [];
rest_Vm = [];
% input_R = [];
...
for i_exp = 1:length(files_to_take)
    try
        ...
        outwave_file_start= char(files_in_folder(startsWith(files_in_folder, start_str_v)));
...
%         end
        ...
        [AM, W, After, vm] = Analysis_workflow.Pairing_analysis(experimenter_ID,  data,I_traces, do_plotting, this_exp, p);
...
for iname = 1:length(NAMES)
   ...
   rep_this_name = repmat(this_name, length(AFTER),1);
    new_names = [new_names,rep_this_name];
end
    
names_table = table(new_names);
After_table = table(AFTER);
amp_AP_table = table(amp_AP);
width_AP_table = table(width_AP);
Vm_table = table(rest_Vm);



% pulses_str = pulses2str(pulses);
% create table with AP firing rate
this_date_table = table(repmat(recording_date, size(AFTER,1),1), 'VariableNames', {'Date'});
...
% After_table = array2table(AFTER, 'VariableNames',{'After Hyper'});
...
%%
filename_xlsx = os.path.join(GC.path_putput_AP_analysis.(experimenter_ID),'Pairing_analysis.xlsx');


%%
this_table = [this_date_table, names_table, amp_AP_table,width_AP_table, Vm_table, After_table];
...
%     range3 =  ['B',char(num2str(sz_or3+5))];
    ...
    % Write table
    writetable(this_table,filename_xlsx,'Sheet',1,  'WriteMode', 'append') 
...
%     writetable(DER_phase_table,filename_xlsx,'Sheet','Phase DER trace', 'Range', range3)   
    ...
    if overwrite
        ...
        writetable(this_table,filename_xlsx,'Sheet',1, 'Range', 'B1', 'WriteMode', 'overwrite')
