...
AP_V_thres = []; %  This defines the voltage at which the AP started (change of more than 5mV/ms)
width_AP = [];
rest_Vm = [];
input_R = [];
Firing_threshold = [];
...
%             continue
        ...
        [FR, AM,AM_fromAPthr, W, Vm, Ri, thr, sr, bump,  pulses, sag_d, a_phase, ap_phase, der_phase, vel_depo, vel_repo, tau_mb, adapt_index, burst_freq, flag,AP_voltage_thr] = Analysis_workflow.AP_analysis(experimenter_ID,  data,I_traces, do_plotting, this_exp, p,take_astrocyte_shape);
%         all_data = Analysis_workflow.AP_analysis(experimenter_ID,  data,I_traces, do_plotting, this_exp, p);
        ...
        AP_V_thres = [AP_V_thres,AP_voltage_thr];
        width_AP = [width_AP, W];
        rest_Vm = [rest_Vm, Vm];
        input_R = [input_R, Ri];
...
V_AP_thres_table = array2table(AP_V_thres', 'VariableNames', {'AP_Voltage_threshold'}); 
...
%%
this_table = [this_date_table, names_table,FR_AP_table, amp_AP_table,amp_AP_thr_table, width_AP_table, V_AP_thres_table,Vm_table, Ri_table, Firing_threshold_table, ...
