function varargout = SAG_analysis(experimenter_ID,V_traces, I_traces, do_plotting, name, p)
% Parse inputs
SR = p{1};
...
pulses = NaN(1, n_steps);
for i_p = 1 : n_steps
    this_pulse_points = unique(I_traces);
...
this_duration_step = round(sum(I_traces == this_pulse_points(2))) +1;
duration_ms = 1000 * this_duration_step / (size(I_traces,1) / p{2});

if n_steps ~= p{5} || p{6} ~= unique(diff(pulses)) || p{1} ~= (size(I_traces,1) / p{2}) || p{4} ~= duration_ms
...
%     if i_data == 1
       % Sag ratio
      vm = median(V_traces(1:finish_bsl-1,[i_data]));
