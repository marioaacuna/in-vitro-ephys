function varargout = PPR_analysis(~,I_traces, V_traces, do_plotting, name, p)
% Parse inputs
SR = p{1};
start_stim = p{2} / 1000;
...
%     freqz(b,a,32000,fs)
    ...
    if isempty(peaks) || loc(1) > 0.020 % if there's no peak or if the first peak is found later than 20ms
        ...
        peak_to_decay = 2; % take the second peak
...
%     above0 = find(data_start_stim > 0,1, 'first');
    percent_i = first_amp(1) * 0.2;
    percent_f = first_amp(1) * 0.8;
...
%     this_Ra = NaN;
    ...
    Ra (i_data) = this_Ra;
    % EPSP parameters
    Amp(i_data) = first_amp * 1000; % in pA
