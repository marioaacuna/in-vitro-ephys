...
for k = vec
	...
	if nargout<3 % faster:
        try
            ...
            if strcmpi(ME.message, 'First input <X> must be a cell array of char row vectors (1xN char).')
                % Convert everything to a string and then to character array
                x = cellfun(@string, X(ndx,k), 'UniformOutput',false);
                % Replace missing values with empty string
                try
                    x(cellfun(@(x) ismissing(x), x)) = {""};
                catch ME
                    if strcmp(ME.identifier, 'MATLAB:cellfun:NotAScalarOutput')  % This column cannot be sorted
                        continue
                    else
                        rethrow(ME)
                    end
                end
                x = cellfun(@char, x, 'UniformOutput',false);
                % Sort values
                [~,ids] = natsort(x,varargin{:});
...
% Return as a table, if inpput was a table
if input_is_table
    X = cell2table(X, 'VariableNames',column_names);
end
