function DONE =  run_EPSC_analysis(experimenter_ID, recording_date, do_plotting, varargin)
clc
DONE = 0;
p = varargin;
overwrite = p{8};
...
for i_name = 1:length(the_names)
    ...
    for i_trial = 1:n_trials
        ...
        this_table = array2table([sw_id,amp, slp, Ri, Ra, I_hold, on, wd, rt, dt], 'VariableNames', data_fieldnames);
...
%         I_h = cell(0,0);
        ...
        for i_exp = 1:length(files_to_take)
            try
                ...
                [sweep_id, amp, rise, decay, slope, wd, on, i_hold, ri, ra] = Analysis_workflow.EPSC_analysis(experimenter_ID,  data, V_traces, do_plotting, this_exp, p);
...
%         filename_xlsx = os.path.join(GC.path_putput_AP_analysis.(experimenter_ID), this_folder,  'AP_frequency.xlsx');
        ...
        %%
        this_table = [this_date_table, names_table,Sweeps_table, Amp_table, Slope_table,Ri_table, Access_table,I_hold_table, Onset_table, Width_table, Rise_table, Decay_table];
